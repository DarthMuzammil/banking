...
The frontend mirrors this in reverse: page → `bankingApi` facade → `fetch` client → JSON → React state.
...
sequenceDiagram
    ...
    API->>H: LoginQuery
    H->>CR: GetByEmailAsync
    CR->>DB: SELECT Customer + Role
    DB-->>CR: row
    H->>CR: GetPasswordHashAsync
    CR->>DB: SELECT CustomerAuth
    H->>H: BCrypt verify
    H->>JWT: GenerateToken(customer)
    H->>RT: IssueAsync(customer)
    RT->>DB: INSERT RefreshToken
    H-->>API: Result<AuthResponseDto>
    API-->>UI: { token, refreshToken, customer }
    UI->>UI: localStorage + AuthContext
```
...
```mermaid
sequenceDiagram
    participant UI as Account detail page
    participant API as AccountsController
    participant F as IdempotencyFilter
    participant H as DepositCommandHandler
    participant AR as AccountRepository
    participant TR as TransactionRepository
    participant AL as AuditLogRepository
    participant DB as SQL Server

    UI->>API: POST /deposits + Idempotency-Key
    API->>F: check cache
    alt cache hit
        F-->>UI: cached JSON (no double credit)
    else cache miss
        F->>H: DepositCommand
        H->>AR: GetByIdAsync (read balance + RowVersion)
        H->>DB: BEGIN TRANSACTION
        H->>AR: UpdateBalanceAsync (WHERE RowVersion = @rv)
        H->>TR: InsertAsync Transaction
        H->>AL: AppendAsync AuditLog
        H->>DB: COMMIT
        H-->>API: DepositResponseDto
        F->>DB: store idempotency record
        API-->>UI: 200 OK
    end
```
...
```mermaid
flowchart TD
    A["GET /api/admin/customers?search="] --> B{JWT role = Staff?}
    B -->|No| C[403 Forbidden]
    B -->|Yes| D[SearchCustomersQueryHandler]
    D --> E[CustomerRepository.SearchAsync]
    E --> F[(Customer table)]
    G["GET /api/admin/customers/{id}/accounts"] --> H[GetCustomerAccountsAdminQueryHandler]
    H --> I[AccountRepository.GetByCustomerIdAsync]
```
